    ...
    , m_offset(offset)
{
    ...
    {
        OGRE_EXCEPT(Ogre::Exception::ERR_INVALID_STATE, "Stream size cannot be lower than offset");
    }

    if ((underlyingStreamSize - offset) < size)
    {
        OGRE_EXCEPT(Ogre::Exception::ERR_INVALID_STATE, "Stream size cannot be lower than offset + size");
    }
